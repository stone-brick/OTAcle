...
#[tauri::command]
pub fn find_window_by_title(title: String) -> Result<Option<i64>, String> {
...
#[tauri::command]
pub fn find_first_window_by_title(title: String) -> Result<Option<i64>, String> {
    let results = input::find_windows_by_title(&title);
    Ok(results.into_iter().next().map(|h| h as i64))
}

#[tauri::command]
pub fn find_windows_by_title(title: String) -> Result<Vec<i64>, String> {
    Ok(input::find_windows_by_title(&title).into_iter().map(|h| h as i64).collect())
}

#[tauri::command]
pub fn find_first_window_by_title_contains(title: String) -> Result<Option<i64>, String> {
    let results = input::find_windows_by_title_contains(&title);
    Ok(results.into_iter().next().map(|h| h as i64))
}

#[tauri::command]
pub fn find_windows_by_title_contains(title: String) -> Result<Vec<i64>, String> {
    Ok(input::find_windows_by_title_contains(&title).into_iter().map(|h| h as i64).collect())
}

#[tauri::command]
pub fn find_window_by_class(class_name: String) -> Result<Option<i64>, String> {
    Ok(input::find_window_by_class_name(&class_name).map(|h| h as i64))
}

#[tauri::command]
pub fn find_windows_by_pid(pid: u32) -> Result<Vec<i64>, String> {
    Ok(input::find_windows_by_pid(pid).into_iter().map(|h| h as i64).collect())
}

#[tauri::command]
pub fn find_windows_by_exe(exe_name: String) -> Result<Vec<i64>, String> {
    Ok(input::find_windows_by_exe(&exe_name).into_iter().map(|h| h as i64).collect())
}

#[tauri::command]
pub fn find_window_by_hwnd(hwnd: i64) -> Result<Option<i64>, String> {
